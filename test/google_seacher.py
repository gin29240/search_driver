...
class Scholar:
    ...
    def __search_onepage(self):
        """爬取搜索结果的的信息"""
        results = []
        gs_scl = self.driver.find_elements(By.XPATH, "//span[@jscontroller='msmzHf']")
        for i, item in tqdm(enumerate(gs_scl)):
            item.find_elements(By.XPATH, "//a[@jsname='msmzHf']")
            gs_rt = item.find_element(by=By.TAG_NAME, value='h3')
            gs_rt_a = gs_rt.find_element(by=By.TAG_NAME, value='a')
            # if self.check_element_exist(check_type='TAG_NAME',value='a', source=gs_rt.get_attribute('innerHTML'))
            # else None
            # 站点标题
    ...
    def search(self, keywords, sort_bydate=False, as_ylo='', as_yhi='', max_pages=100, delay=0):
        ...
        url = f'https://www.google.com/search?{sort_bydate}&hl=zh-CN&as_sdt=0%2C5&q={keywords}&btnG=&as_ylo={as_ylo}&as_yhi={as_yhi}'
        # 打开Google Scholar网站
        self.driver.get(url)
...
if __name__ == '__main__':
    ...
    keywords = "inurl:index.php?s=/Home"
    as_ylo = '1900'
    as_yhi = '2024'
    max_pages = 10
    sort_bydate = 'y'
    # max_pages = input('>> 请输入爬取多少页(最多为100): ').strip() or '100'
    out_filepath = '_'.join(keywords.replace('"', '').
                            replace(':', '').replace('.', '').
                            replace('?', '').replace('/', '').
                            split())

    scholar = Scholar(out_filepath)  # 创建Scholar 类  新建关键字文件夹
    scholar.start_browser(wait_time=60)  # 创建webdriver，等待加载完成
    results = scholar.search(keywords, sort_bydate, as_ylo, as_yhi, max_pages=int(max_pages),
                             delay=random.randint(0, 0))
    scholar.close_browser() # 关闭浏览器
